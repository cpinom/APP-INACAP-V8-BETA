...
export class BuscadorComponent implements OnInit {
  ...
  @Input() rol!: string;
  @Output() onOfertaTap: EventEmitter<any> = new EventEmitter();
  programa: any;
  ...
  async filtrar() {
    ...
    try {
      ...
      const { carrera, region, comuna, tipos } = this.filtros.usuario;
      const filtro = this.filtros.filtro || '';
      const page = this.filtros.page;

      const result = await this.api.getEmpleos(carrera, region, comuna, tipos, filtro, page);
  ...
  get esExalumno() { return this.rol == 'exalumno'; }
